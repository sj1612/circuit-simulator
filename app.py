...
def build_node_mapping(components, wires, ground_component_id):
    ...
    return term_to_node, total_nodes
...
def solve_steady_state(components, term_to_node, total_nodes, frequency=0.0):
    ...
    for comp in components:
        ...
        terms = comp.get("terminals", [])
        if len(terms) < 2:
            continue  # skip ground or incomplete components
        ...
        # Compute equivalent admittance
        if ctype == "resistor":
            R = float(comp.get("value", 1.0))
            g = 1.0 / R if R != 0 else 1e12
        elif ctype == "capacitor":
            if frequency == 0: 
                continue
            C = float(comp.get("value", 0.0))
            g = 1j * omega * C
        elif ctype == "inductor":
            L = float(comp.get("value", 0.0))
    ...
    for comp in components:
        ...
        Va, Vb = complex(node_voltages[na]), complex(node_voltages[nb])
        Vab = Va - Vb
        I = 0j

        if ctype == "resistor":
            I = Vab / float(comp.get("value", 1.0))
        elif ctype == "capacitor":
            C = float(comp.get("value", 0.0))
            I = (1j * omega * C * Vab) if frequency > 0 else 0
        elif ctype == "inductor":
            L = float(comp.get("value", 0.0))
